...
import { RhDynamicFormItem } from '../../RhDynamicToolkit/form';
import { isNumberType } from '../../RhDynamicToolkit/utils';
...
export default function EditableCell(props: PropsWithChildren<CellProps>) {
  ...
  const input = useMemo(() => {
    if (dataIndex) {
      return (
        <RhDynamicFormItem
          widgetProps={{ id: dataIndex, ...quickEdit }}
          autoFocus={true}
          className="fixedError"
        />
      );
    }
    return null;
  ...
  if (quickEdit) {
    return (
      ...
      >
        <div className={isEditing ? '' : 'editable-border'}>
          {allowInput ? input : props.children}
        </div>
      </td>
    );
  }
