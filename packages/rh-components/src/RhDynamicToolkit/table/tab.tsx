...
function RhDynamicTabTable({
  ...
  const getList = useCallback(
    async (tableParams = {}) => {
      ...
      if (Array.isArray(dataSourceApi)) {
        ...
        if (data.axisCount > 3) {
          const axisList = cloneData.filter(
            (d) =>
              d.propertyType === 'axis' && d.propertyId.indexOf('X_') === 0,
          );
  ...
  const handleAdd = useCallback(() => {
    ...
    const btn: HTMLElement = document.querySelector(
      '.dynamicComplexTable .ant-table-thead button.ant-btn-dashed',
    );
    btn.click?.();
  }, []);
  ...
  const handleEditorChange = useCallback(
    (recordList, type = 'change') => {
      ...
      if (diffKeys.length > 0) {
        ...
        diffKeys.length === 0 &&
        recordList.length === dataSourceKeys.length
      ) {
        // 数据没有变化
      ...
      if (type === 'change') {
        ...
        if (keys.length > 0) {
          cacheRef.current[tabValue + '_selected'] = [
            ...cacheSelectedKey,
            ...keys,
          ];
        }
      ...
      const filterCacheData = cacheData.filter(
        (item) => !valueKeys.includes(item['id']),
      );
    ...
    (tableSchema: AnyObject, id?: string) => {
        ...
        customToolBar = toolbar?.body.map((item) => {
          if (item.renderType === 'radio') {
            ...
            if (item?.source && (data || tableSchema?.data)) {
              valueEnum = resolveVariableAndFilter(
                item.source,
      ...
      if (editable) {
        ...
        if (recordCreatorProps !== false && recordCreatorProps !== null) {
          const { buttonProps, creatorButtonText, visibleOn } =
            recordCreatorProps || {};
          let visible = true;
          if (visibleOn) {
            const val = argRef.current[argRef.current._radioKey];
            visible = evalExpression(visibleOn, {
              [argRef.current._radioKey]: val,
            });
          }
          ...
          <EditableProTable<any>
            ...
            rowSelection={{
              ...
              getCheckboxProps: (record) => ({
                disabled:
                  argRef.current[radioKey] === 'pmc' ||
                  argRef.current[radioKey] === 'macro', // Column configuration not to be checked
                id: record.id,
              }),
            }}
            pagination={false}
      ...
      if (block?.type === 'tab-table') {
        ...
        if (block?.tabs && (block?.tabs.valueEnum || block?.tabs.source)) {
          if (!argRef.current._tabKey) {
            const q =
              data?.tabList?.length > 0 ? data?.tabList[0]['value'] : '';
          ...
          if (block?.tabs.source && (data || block?.data)) {
            const valueEnum = resolveVariableAndFilter(
              block?.tabs.source,
              data || block?.data,
            );
            if (valueEnum?.length > 0) {
  ...
  const handleConfirm = useCallback(() => {
    ...
    const result = tabKeyList.map((tabKey) => {
      ...
      const formatObjList = selectedRows.map((row) => {
        ...
        if (
          !rowDataNotNil({ propertyId, name, address, valueType, authority })
        ) {
          return null;
        }
        const obj = {
