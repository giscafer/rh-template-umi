import { CheckOutlined, MenuOutlined, SettingOutlined, SwapOutlined } from '@ant-design/icons';
import { ProColumns, useRefFunction } from '@ant-design/pro-components';
import { genColumnKey } from '@ant-design/pro-table/lib/utils';
import { Button, Checkbox, Popover, Space, Switch, Table, TableColumnType, Tooltip } from 'antd';
...
const SortableItem = SortableElement((props: Record<string, any>) => <tr {...props} />);
const SortableBody = SortableContainer((props: Record<string, any>) => <tbody {...props} />);
...
const ColumnSetting: React.FC<ColumnSettingProps> = (props:any) => {
  ...
  const columnChangeHandle = useCallback(
    (settingColumns: any[]) => {
      const c = settingColumns.map((item: { [x: string]: any; index: any; fixedLeft: any; fixedRight: any; hideInTable: any; }) => {
  ...
  const settingTableColumns = useMemo(() => {
    return [
      ...
      },
      {
        ...
        dataIndex: 'fixedLeft',
        width: 60,
        render: (text: any, record: { fixedLeft: boolean | undefined; dataIndex: any; }) => (
          <Switch
      ...
      },
      {
        ...
        width: 60,
        render: (text: any, record: { fixedRight: boolean | undefined; dataIndex: any; }) => (
          <Switch
            checkedChildren={<CheckOutlined />}
        ...
        render: (text: any, record: { hideInTable: any; dataIndex: any; }) => {
          return (
            <Checkbox
              ...
              onChange={(e) => {
                changeSettingHandle(record.dataIndex, 'hideInTable', !e.target.checked);
              }}
            >
              显示
            </Checkbox>
  ...
  const DraggableContainer = (props: JSX.IntrinsicAttributes & JSX.IntrinsicClassAttributes<React.Component<SortableContainerProps, any, any>> & Readonly<SortableContainerProps>) => (
    <SortableBody
      useDragHandle
      disableAutoscroll
    ...
    <Popover
      ...
      content={
        <div className="rh-table-column-setting-content">
          ...
          />
          <div className="flex-end p2 ph3" style={{ borderTop: '1px solid #D6D6D8' }}>
