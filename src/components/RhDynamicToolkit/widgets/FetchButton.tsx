...
// params 接口请求参数
function FetchButton({ title, dataIndex, params = { url: '', data: {} }, feedback = { success: '', error: '' } }) {
  ...
  return (
    <div className="ant-row ant-form-item m-0">
      <div style={{ paddingTop: '14px' }}>
        ...
        {feedback?.success && showFeedback === 'success' && <span className="color-primary">{feedback.success}</span>}
        {feedback?.error && showFeedback === 'error' && <span className="red">{feedback.error}</span>}
      </div>
    </div>
  );
}
