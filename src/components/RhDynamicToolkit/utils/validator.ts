...
const getMessage = (fieldProps: AnyObject) => {
  return `${
    ['text', 'textarea', 'input'].includes(fieldProps.valueType || fieldProps.renderType) ? '请输入' : '请选择'
    ...
    for (const item of validator) {
      ...
      if (type === 'range') {
        const validatorFunc = async (rule, v: string) => {
          ...
          if (flag) {
            return Promise.reject(`${message || `请输入${value[0]}~${value[1]}之间的数字`}`);
          }
          return;
        };
        rules.push({
      ...
      if (type === 'min') {
        const validatorFunc = async (rule, v: number) => {
          ...
          if (flag) {
            return Promise.reject(`${message || `请输入大于等于${value}的数字`}`);
          }
          return;
        };
        rules.push({
      ...
      if (type === 'max') {
        const validatorFunc = async (rule, v: number) => {
          ...
          if (flag) {
            return Promise.reject(`${message || `请输入小于等于${value}的数字`}`);
          }
          return;
        };
        rules.push({
        ...
        const dynRule = (form) => {
          return {
            async validator(rule, v: string) {
              if (typeof v === 'undefined') return;
              let flag = !evalFormula(value, { ...form.getFieldsValue(), value: v });
              // validateFirst 有bug，先临时解决
              if (fieldProps.required) {
                flag = flag && isNotEmpty(v);
              }
          ...
          return {
            async validator(rule, v: string) {
              ...
              try {
                ...
                const fn = new Function('k', 'v', 'vals', `return (${value})(k, v, vals)`);
