...
import { CaretDownOutlined, EllipsisOutlined, PlusCircleOutlined } from '@ant-design/icons';
...
const RhTree = (
  ...
  const handleTreeNodeClick = (e: React.MouseEvent, node: Partial<any & { name: string }>) => {
    e.stopPropagation();
  ...
  const handleTreeNodeSelect = (keys: (string | number)[], info?: { nativeEvent: MouseEvent }) => {
      ...
      <span className="action" onClick={(e) => e.stopPropagation()}>
        <>
          {addMenuRender
            ...
            : showAddMenu && <IconFont type="icon-tree-create" className="icon" />}
  ...
  const renderTree: any = (leafList: ILeafNode[], idx: number, parentId: Key, isCreate: boolean) => {
    if (debug) {
    ...
    const tree = leafList.map((leaf: any) => ({
      ...
      // icon: leaf.icon || '',
      className: [
        !leaf.parentId && !(leaf.children && leaf.children.length) ? 'rh-tree-parent-no-children' : '',
      ...
      title: !leaf.isEdit ? (
        <div className="tree-leaf">
          {iconRender ? (
            ...
            leaf.icon && <IconFont type={leaf.icon as string} className="leaf-icon" />
          )}
          ...
          {menuProps && editable && menuProps.types[leaf.type || 'default'] && (
            ...
            >
              <a onClick={(e) => e.stopPropagation()}>
                {menuProps.trigger || <EllipsisOutlined rotate={90} className="text-base" />}
              </a>
            </Dropdown>
          )}
        </div>
      ...
      ? tree.concat({
          ...
          title: (
            <Input
              ...
              ref={inputNode}
              value={inputValue}
              placeholder={nodeType === 'NODE' ? '请输入节点名称' : '请输入属性名称'}
              // placeholder="输入限制为10个字符"
              suffix={<span>{inputValue.length}/10</span>}
              onChange={({ currentTarget }) => {
  ...
  const titleRenderFn = (nodeData: any) => {
    ...
    return (
      <div className="flex items-center">
        ...
        <div className="ant-tree-title-extra">{isRenderNumb ? `（${nodeData?.number}）` : ''}</div>
          ...
          >
            <div className="flex justify-center items-center">
              <div className="ant-tree-statistics-icon">
                <RhIcon fontSize={16} src={statisticsAllProps?.leftIcon || ''} />
              </div>
